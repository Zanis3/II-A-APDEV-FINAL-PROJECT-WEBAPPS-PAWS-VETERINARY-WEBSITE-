<?php
    ...
    if (basename("dashboard/dash-template/DentalChart.php") == basename($_SERVER["SCRIPT_FILENAME"])){
        if($role == 'admin'){
...
<body>
    <div class="chart">
        <canvas id="dentalChart" width="250" height="250"></canvas>
        <script>
            var ctx = document.getElementById('dentalChart').getContext('2d');
            var data = {
                labels: [ 'Pending', 'Done', 'Canceled' , 'Empty' ],
                datasets: [{
                    label: 'Dental Chart',
                    data: [5, 18, 3, 76], // Example data
                    backgroundColor: [
                        'rgb(6, 78, 59, 1)',
                        'rgb(6, 78, 59, 0.8)',
                        'rgb(6, 78, 59, 0.6)',
