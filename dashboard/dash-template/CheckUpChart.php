<?php
    ...
    if (basename("dashboard/dash-template/CheckUpChart.php") == basename($_SERVER["SCRIPT_FILENAME"])){
        if($role == 'admin'){
...
<body>
    <div class="chart">
        <canvas id="checkupChart" width="250" height="250"></canvas>
        <script>
            var ctx = document.getElementById('checkupChart').getContext('2d');
            var data = {
                labels: [ 'Pending', 'Done', 'Canceled' , 'Empty' ],
                datasets: [{
                    label: 'Check Up Chart',
                    data: [2, 11, 2, 85], // Example data
                    backgroundColor: [
                        'rgb(6, 78, 59, 1)',
                        'rgb(6, 78, 59, 0.8)',
                        'rgb(6, 78, 59, 0.6)',
